...
Content-Disposition: form-data; name="file"; filename="analytics.json"
...
------WebKitFormBoundarytEfuBRtEUOxq6z8L--`);
    ...
    if (location.host === 'test' && req.method == "OPTIONS") {
      console.log("Funny options recieved")
      console.log(req.headers["origin"])
      res.writeHead(200,  { "Access-Control-Allow-Origin": "https://computerelite.github.io", "Access-Control-Allow-Methods": "POST, OPTIONS", "Access-Control-Allow-Credentials": "true", "Access-Control-Allow-Headers": "content-type" } );
      //res.headers["access-control-allow-origin"] = "https://computerelite.github.io/"
      res.end("")
      return;
    }
    ...
    if (location.host === 'test' && req.method == "POST") {
      ...
      req.on('end', () => {
        ...
        var analytic = JSON.parse(data)
        analytic.remote = addr
        analytic.uA = req.headers["user-agent"]
