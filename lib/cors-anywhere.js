  ...
  return function(req, res) {
    ...
    if (location.host === 'postissue' && req.method == "POST") {
      ...
      if(issuesReported.includes(addr)) {
        res.end(`{'msg': 'Issue already reported from this IP', 'ip': '${addr}'}`);
        return;
      }
      ...
      if(issuesReported.length >= minIssues) {
        try {
            ...
            embeds: [
              {
                ...
                description: "10 or more users reported errors with their requests",
                author: {
          ...
          var xhr = new XMLHttpRequest();
          xhr.open("DELETE", process.env.restarturl, true);
          xhr.setRequestHeader('authorization', process.env.authorization);

          xhr.send(JSON.stringify({
            branch: "master"
          }));
        } catch (e) {
          res.end(JSON.stringify({'msg': 'An error occured while trying to report an issue', 'ip': addr}))
          console.error(e)
        }
      } else {
